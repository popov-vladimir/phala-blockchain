...
pub struct Auction {
    bets: BTreeMap<AccountIdWrapper, u32>,
    ...
    fn handle_command(&mut self, _origin: &chain::AccountId, _txref: &TxRef, cmd: Command) -> TransactionStatus {
        match cmd {
            ...
            Command::PlaceBet { value } => {
                ...
                self.bets.insert(current_user, value);

                if value > self.winning_bet {
                    self.winning_bet = value.clone();
                    self.winner = current_user;
                }
